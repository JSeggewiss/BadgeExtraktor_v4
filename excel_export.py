...
COLUMNS = ["Nachname", "Vorname", "Institution", "Ort", "E-Mail", "Quelle"]
...
def save_to_excel(records: list[dict], output_path: str | Path) -> Path:
    ...
    title_cell = ws.cell(row=2, column=2, value="Badge-Kontakte")
    ...
    for row_idx, record in enumerate(records, start=HEADER_ROW + 1):
        ...
        for col_idx, col_name in enumerate(COLUMNS, start=2):
            ...
            if col_name == "Quelle" and cell.value and cell.value.startswith("http"):
...
def append_to_excel(record: dict, output_path: str | Path) -> Path:
    """Fügt einen einzelnen Datensatz an eine bestehende Datei an (oder erstellt sie neu)."""
    output_path = Path(output_path)
    if not output_path.exists():
        return save_to_excel([record], output_path)

    # Datei neu aufbauen mit allen Daten
    existing = load_records(output_path)
    existing.append(record)
    return save_to_excel(existing, output_path)
